...
這個儲存庫是一套以 [chezmoi](https://www.chezmoi.io) 管理的跨平台 AI 開發環境與開發工具系統。
它透過單一真實來源，協調 Claude Code、Codex、GitHub Copilot 和 VS Code 的設定，同時保留
各工具的原生格式，以及應用程式自行管理的設定。

此外，這個系統也提供只在本機生效的個人／公司情境、獨立的專案連續性控制、跨平台 worktree
工具，以及驗證產生設定的回歸測試。
...
- **三個 AI 工具需要使用部分相同的設定，但各自要求不同的檔案與格式。** 共用內容只寫一份，
  ...
  產生多份內容。
- **同一台電腦可能同時處理個人與公司工作，而且兩者的慣例不同。** 兩個只在本機生效的選擇器
  分別決定工作情境，以及是否啟用自動的專案連續性。專案連續性只存在於目前工作樹的
  私有交接狀態：它會在 `.project-continuity/state.md` 記錄目標、目前階段、下一步、
  阻塞事項與假設，讓新的 AI 工作階段可以接續任務，而不必重新拼湊背景。啟用時，
  Claude Code 與 Codex 會載入連續性指示；工作階段開始與結束時執行的輔助程式，也可能回報或
  維護這份狀態。停用時，這些自動行為不會輸出內容，也不會建立或修改狀態；專案連續性技能仍可在
  明確要求時使用。

  四種組合在產生時都從同一組來源檔案組合，不會複製四套 profile 目錄結構。因此，公司工作在
  產生提交訊息、註解，以及由 `worktree-task-workflow` 技能發布的分支與 merge request
  文字時，預設會使用繁體中文；個人工作則維持英文。儲存庫或專案本身的指示仍然優先於
  本機選擇的情境。
- **不同電腦的設定會逐漸出現差異，而且不同作業系統的相同設定位於不同路徑。** Template 讓
  ...
  設定時不會順便安裝軟體。
- **有些受管理檔案也會被使用它們的應用程式改寫。** 同一個設定檔可能同時包含
  應該同步到所有電腦的內容，以及應用程式自行記錄的內容；整個檔案直接覆寫會破壞後者。以 Claude
  Code 的 `settings.json` 為例，儲存庫只管理少數應長期同步的設定鍵，並將它們合併到 Claude
  已寫入的內容中，讓模型、推理程度與主題等設定留在本機。
...
要組合哪一個情境層、是否啟用連續性指示與回報，以及支援的產出物預設使用哪種語言。
