...
`disable-model-invocation: true` is set deliberately: this skill has side effects (staging, committing), so it should only run when the user explicitly types `/git-commit-action`, never auto-triggered by Codex on its own judgment.
