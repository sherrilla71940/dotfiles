...
Shared conventions for writing a Conventional Commit message. Both organic commits (Codex committing on its own judgment) and the `git-commit-action` skill draw from this file so the style stays in one place.

`user-invocable: false` is set deliberately: this skill is pure reference material with no workflow of its own, so it's hidden from the slash-command menu and only loads when Codex decides it's relevant (or via the Skill tool), never by the user typing a slash command for it.
