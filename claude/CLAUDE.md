...
- Subagents do not inherit the parent conversation or the project's auto memory (a fork is the exception). They **do** load the CLAUDE.md hierarchy — except the built-in `Explore` and `Plan` agents, which skip it. So brief each one with the task-specific facts and constraints that aren't guaranteed to load — proportionate to the task, not a blanket context dump.
