...
Docs say auto memory is shared across worktrees via a git-derived project key (current per docs, re-confirmed 2026-08-05). On this Windows machine that's empirically false: worktrees get their own separate, empty memory folders instead of sharing the main checkout's. Root cause: a **drive-letter case-sensitivity bug** in Claude Code's project-key encoding (main checkout's key is lowercase `c--...`, worktree-derived keys are uppercase `C--...`), not a launch-location mistake and not the same bug as `anthropics/claude-code#39920` (closed "not planned" — describes the opposite symptom, worktrees over-sharing into main, not under-sharing).

Before troubleshooting a worktree memory issue, re-fetch **<https://code.claude.com/docs/en/memory#storage-location>** directly (not from recall) — it documents `autoMemoryDirectory` (used below) and may have changed since this note was written.

<!--
provenance (2026-08-05): confirmed via disk evidence, not just recall. On this repo's ~/.claude/projects/ folders: main checkout = `c--Users-...-taoyuansewer2` (lowercase c); two real, legitimate linked worktrees (confirmed via `git worktree list` / `git rev-parse --show-toplevel`) = `C--Users-...-taoyuansewer2-worktrees-<branch>` (uppercase C). `git rev-parse --git-common-dir` run from inside a real worktree correctly resolves to the main checkout's .git — git itself isn't confused, so the bug is in Claude Code's key encoding, not git. Hypothesis: a session that captures the drive letter with different casing than the session that established the main folder (plausible depending on shell — Git Bash vs cmd/PowerShell format drive letters differently) lands on a different, non-matching key for the identical physical directory. Separately (and NOT part of this bug): a 5th on-disk folder, `C--...-worktrees` with no branch suffix, is fully explained by a session having been launched directly in the plain non-git container folder — documented, expected behavior for "outside a git repo", not an anomaly. Verified `anthropics/claude-code#39920` via GitHub API and WebFetch same day; it describes the opposite symptom (collapsing into main = over-sharing) so it does not explain this mechanism — don't cite it as if it does. If re-diagnosing, look for a Windows drive-letter-casing issue specifically, or re-run this test fresh.
-->


- **Fix:** set `autoMemoryDirectory` in `.claude/settings.local.json` at the **base repo checkout's root only** — e.g. pointing at its existing memory folder, `~/.claude/projects/<base-repo-encoded-path>/memory`. Since Claude Code v2.1.211, `settings.local.json` "resolves through worktrees to the main checkout, so one file covers sessions started in any subdirectory or worktree of the repository" — do **not** duplicate this file into each worktree; that creates a second, lower-priority source of truth for no benefit (repo root wins on conflicting non-permission keys anyway). Check the installed version (`claude --version`) before relying on this — pre-2.1.211 the file lived only in the starting directory, so older installs genuinely do need a per-worktree copy.
...
- **Exception:** a directory that is _not_ itself part of a git repository (e.g. a plain folder one level above several worktrees, used only as a launch point) does NOT get the worktree-resolution behavior — its `settings.local.json` stays local to that exact directory and needs its own `autoMemoryDirectory` entry. Per current docs (re-confirmed 2026-08-05), two more cases also stay local to the start directory rather than resolving to repo root: when the repo root **is** the home directory, and in **Agent SDK** sessions (not plain CLI sessions). Neither is likely to apply to normal repo work, but both are documented exceptions worth knowing about.
...
- **When starting work in a new worktree** (of any repo, not just this one) and `MEMORY.md`/memory content looks unexpectedly empty: don't assume there's no history. Check sibling folders under `~/.claude/projects/` for one matching the same repo without a worktree-path suffix, read memory from there, and confirm/apply `autoMemoryDirectory` at the base repo root (not the worktree) before proceeding.
- **Procedure to apply the fix (base repo root, once):**
  1. If `.claude/settings.local.json` doesn't exist yet at the repo root, create it; if it exists, merge in the key rather than overwriting the file.
  2. Add/update the `autoMemoryDirectory` key to point at the shared memory path.
  3. Check whether `.claude/settings.local.json` is actually gitignored (`git status --porcelain .claude/settings.local.json` — no output means ignored or unchanged-and-tracked; check which). Don't assume a global gitignore rule covers it without checking.
