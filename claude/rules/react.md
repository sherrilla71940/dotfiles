...
- Keep hook declarations together at the top level of the component (after any required constants). Never call hooks conditionally or inside loops, nested functions, or helper functions.
...
- Split out a component when a chunk of JSX represents a distinct, nameable concern (e.g. a list item, a header, a form section) or is reused or likely to be reused elsewhere. Keep trivial, single-use markup inline rather than extracting it solely to shorten the parent.
