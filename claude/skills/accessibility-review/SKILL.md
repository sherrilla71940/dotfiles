...
description: 'Review, design, and implement web accessibility improvements for frontend UI. Use when asked for a11y review, WCAG checks, keyboard/focus behavior, ARIA, forms, alt text, live regions, accessible React/Vue/Angular patterns, or accessibility testing with axe, pa11y, Lighthouse, or screen readers.'
---

# Accessibility Review

Use this skill for focused accessibility work on web interfaces. It turns WCAG guidance into practical implementation, review, and testing steps without requiring a separate accessibility persona agent.

## When to Use This Skill

- Reviewing a UI, diff, component, page, or flow for accessibility issues
- Implementing accessible dialogs, menus, tabs, forms, tables, charts, or dynamic updates
- Fixing keyboard traps, missing focus restoration, weak focus indicators, or poor tab order
- Checking labels, names, roles, states, error messages, live regions, and alt text
- Creating accessibility acceptance criteria, QA checklists, or verification steps
- Running or interpreting axe, pa11y, Lighthouse, keyboard-only, or screen reader smoke tests

## Review Workflow

1. Start with the affected user flow and identify the content, controls, status changes, and form states a user must understand or operate.
2. Check semantic structure first: landmarks, headings, labels, tables, lists, native controls, and document language.
3. Walk the flow with keyboard only. Verify tab order, activation keys, focus visibility, Escape behavior, focus trapping, and focus restoration.
4. Inspect name, role, value, and state. Confirm accessible names match visible labels and ARIA reflects actual UI behavior.
5. Review visual and media requirements: contrast, target size, reflow, zoom, reduced motion, captions, and alt text.
6. Use automated tools as support, then add manual verification notes for issues tools cannot prove.

## Testing Commands

Use these when the project supports them or when a local page is available:
...
Automated scans are not enough. Pair them with at least one keyboard-only walkthrough for the affected workflow, and add a screen reader smoke test for critical paths when practical.

## Review Output

Lead with the highest-impact issues and reference the affected file or component. Group findings by severity, explain user impact, suggest a concrete fix, and include verification steps.

## Full WCAG Reference

The following reference was moved out of the always-on instruction so it loads only when this skill is relevant.
