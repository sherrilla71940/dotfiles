...
3. **Agent View (`claude agents`)** is a dashboard, not an orchestrator. Where you type it doesn't matter. Each background agent it dispatches is a separate process that **starts in your working directory**; per `/docs/en/agent-view`, "before editing files, Claude moves the session into an isolated git worktree under `.claude/worktrees/`" — so worktree creation is lazy, and a read-only background session never gets one. Memory is unaffected either way: main checkout or worktree, it's the same repo, so both **do** get the shared memory (verified). But no dispatched session directs or waits on another, so *you* are the orchestrator.
