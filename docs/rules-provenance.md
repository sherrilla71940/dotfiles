...
Maintainer notes for `home/.chezmoitemplates/rules/`. **This file is never linked into any assistant's
...
`home/.chezmoitemplates/core.md` would be read verbatim by both, wasting Codex's 32 KiB
`project_doc_max_bytes` budget. So maintainer notes for shared files go here, and only
Claude-only files (`home/dot_claude/CLAUDE.md.tmpl`) keep inline HTML comments.
...
- **"Verify version-sensitive tooling details before acting."** — the tool-neutral form of
  a rule that started as a Claude-only instruction. `home/dot_claude/CLAUDE.md.tmpl` keeps the sharper
...
`home/.chezmoitemplates/rules/` was created from two independently maintained trees that had drifted:
...
**No reworded twins.** A rule that names Claude machinery lives in `home/dot_claude/CLAUDE.md.tmpl` only
