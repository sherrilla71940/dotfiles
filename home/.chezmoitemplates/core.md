...
- **A failed lookup is not evidence of absence.** The shell's working directory persists between commands, and a `cd` inside one command carries into every later one, so `ls`/`find`/`grep` can report "No such file" about something that is present. Confirm the working directory before concluding that a file, directory, or match does not exist, and prefer absolute paths — or re-anchoring at the repository root — in any command whose output you intend to rely on. When two sources disagree about whether something exists, suspect that their paths are relative to different bases before believing either: `git status --porcelain` always prints paths relative to the repository root, whatever the working directory is.
