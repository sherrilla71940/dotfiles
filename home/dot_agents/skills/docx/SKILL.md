...
| View embedded images | Unpack, then use the file reader on the files under `word/media/` - see Viewing Embedded Images below |
...
Then use the file reader on each file in `unpacked/word/media/`. Cross-reference `word/document.xml` (`r:embed` / `r:id` → `word/_rels/document.xml.rels`) to know where each image sits in the document if position matters.
...
**Use the file editor directly for string replacement. Do not write Python scripts.** Scripts introduce unnecessary complexity. The file editor shows exactly what is being replaced.
